...
parameters_df <- dfs %>% 
    ...
    dplyr::bind_rows() %>% 
    dplyr::select(SUBMISSION_ID, step, name, relationship, value, unit) %>% 
    magrittr::set_colnames(c(
        "SUBMISSION_ID",
        "STEP_NAME",
        "PARAMETER_NAME", 
...
write_csv(steps_df, "round3_late_steps.csv")
write_csv(parameters_df, "round3_late_parameters.csv")
