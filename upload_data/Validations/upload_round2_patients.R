...
df <- "select patientId, diagnosis, sex, tumorType, tumorPurityPercent, checkpointInhibitor, classIHLAalleles from syn8292741 where round = 2" %>% 
...
allele_df <- df %>% 
    ...
    dplyr::mutate(HLA_ALLELE = stringr::str_c(
...
patient_df <- df %>% 
    ...
    dplyr::filter(TUMOR_TYPE != "NA") %>% 
    dplyr::filter(!is.na(TUMOR_TYPE)) %>% 
