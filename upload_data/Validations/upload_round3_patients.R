...
df <- "select patientId, diagnosis, sex, tumorType, checkpointInhibitor, classIHLAalleles from syn20505381" %>% 
...
patient_df <- df %>% 
    ...
    dplyr::distinct() %>% 
    magrittr::set_colnames(c("PATIENT_ID", "DIAGNOSIS", "GENDER", "TUMOR_TYPE", "CHECKPOINT_TREATED"))
